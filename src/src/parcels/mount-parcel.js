...
export function mountParcel(config, customProps) {
  ...
  const configLoadingFunction = passedConfigLoadingFunction ? config : () => {console.log('dingdingdingding');return Promise.resolve(config)}
  ...
  const bootstrapPromise = loadPromise.then(() => {console.log('该挂载了该挂载了哈哈哈哈');return toBootstrapPromise(parcel, true)});
